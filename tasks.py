...
PACKAGE_NAME = "python-project-gh-workflow"

DEFAULT_DOCKER_REPOSITORY = "galactipy/python-project-gh-workflow"
...
def check_safety(c: Context) -> None:
    ...
    c.run(f"{VENV_BIN}/bandit -ll --recursive python_project_gh_workflow", pty=PTY)
...
def docker_build(
    ...
    """Build Docker images for python-project-gh-workflow."""
    if len(tags) == 0:
...
def docker_remove(
    ...
    """Remove specified Docker images created for python-project-gh-workflow."""
    if len(tags) == 0:
